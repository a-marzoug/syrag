...
| `qdrant` | `pip install "syrag[qdrant]"` | `qdrant-client>=1.16.0` | `QdrantVectorStore` | Local embedded Qdrant storage or remote Qdrant deployments. |
...
| `llamaindex` | `pip install "syrag[llamaindex]"` | `httpx>=0.28.1`, `llama-index-core>=0.14.0` | `LlamaIndexNodeChunker`, `LlamaIndexRetrieverStrategy`, `SyRAGQueryEngine` | Use LlamaIndex node parsers, retrievers, and query engines with SyRAG. |
...
| `all` | `pip install "syrag[all]"` | `chromadb`, `faiss-cpu`, `google-genai`, `httpx`, `langchain-core`, `langchain-text-splitters`, `llama-index-core`, `qdrant-client`, `uvicorn[standard]` | Runtime integrations from `chroma`, `faiss`, `google`, `langchain`, `llamaindex`, `openai`, `qdrant`, and `server` | Convenience install for local development with all runtime integrations. |
...
| OpenAI plus Qdrant | `pip install "syrag[openai,qdrant,server]"` |
...
| LlamaIndex parsing plus OpenAI and Chroma | `pip install "syrag[llamaindex,openai,chroma,server]"` |
