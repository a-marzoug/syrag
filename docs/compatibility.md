...
| `all` | `pip install "syrag[all]"` | `chromadb`, `faiss-cpu`, `google-genai`, `httpx`, `langchain-core`, `langchain-text-splitters`, `llama-index-core`, `uvicorn[standard]` | Runtime integrations from `chroma`, `faiss`, `google`, `langchain`, `llamaindex`, `openai`, and `server` | Convenience install for local development with all runtime integrations. |
