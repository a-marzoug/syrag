...
class HostedQdrantVectorStore(VectorStore):
    """Qdrant Cloud or remote Qdrant vector-store adapter for SyRAG."""
...
def build_vector_store() -> VectorStore:
    return HostedQdrantVectorStore(
        client=QdrantClient(
...
async def build_app() -> SyRAG:
    ...
    syrag = SyRAG(
        title="Hosted Qdrant Support Bot",
        version="0.1.0",
        description="SyRAG backed by hosted Qdrant.",
        settings=Settings(environment=os.environ.get("SYRAG_ENVIRONMENT", "production")),
    )
    ...
    async def ingest(request: IngestRequest) -> IngestRequest:
        return request.model_copy(
            update={
                ...
                "metadata": {"source": "hosted-qdrant-api", **request.metadata},
            }
        )
